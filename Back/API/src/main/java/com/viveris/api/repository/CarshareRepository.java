...
public interface CarshareRepository extends CrudRepository<Carshare, Long> {
	...
	@Query(value = "SELECT Carshare.* FROM Carshare LEFT JOIN Passenger ON(uid=uid_carshare) "
			+ "WHERE (uid_passenger<> :id_user OR uid_passenger IS NULL) "
			+ "AND UID_driver<> :id_user "
